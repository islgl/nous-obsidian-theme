...
### Fixed
- **Mermaid sequence diagrams in dark mode** — actor / labelBox / note boxes were being painted dark (`#30302e`) but Mermaid's SVG-embedded `<style>` forces `text.actor > tspan { fill: black }` (and similarly for labelText / noteText / loopText) with a specificity we can't beat without unhinged selectors. Result: black text on dark box = invisible. Fix inverts the strategy: keep those three element types as **warm cream cards** (`#e8e2d2`) in dark mode too — Mermaid's forced-black tspan text then reads fine against a light card. Only the flowchart / state / class / ER entity boxes get the dark `--mm-surface` treatment; those diagram types don't inject the tspan override, so our `--mm-text` warm-off-white rule cascades into their tspan children normally.
