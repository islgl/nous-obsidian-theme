...
### Changed
- **Mermaid dark mode: terracotta cards + white text**. User brief: edge labels / pie title / legend go white; node boxes become terracotta red. Implemented as:
  - `--mm-text` lifted from `#d5d2c6` to `#f5f4ed` — bright warm white, matching the theme's own text-normal so diagram text reads like the note prose. Applies to all free-floating text in dark mode (edge labels + halo, pie title & legend, sequence message text, gantt task text, cluster labels) AND to text inside the terracotta cards.
  - Flowchart nodes, state-diagram states, class-diagram boxes, ER entities now render on the brand terracotta `#c96442` with a rust `#8f4a31` border; text inside reads at roughly 4.5:1 contrast. ER attribute even rows use a muted `#b25a3a` for alternating-stripe legibility on the terracotta entity card.
  - **Sequence actors / notes / labelBoxes stay as cream cards** — Mermaid forces `#svgid text.actor > tspan { fill: black }` (specificity 1,1,2) that we can't override from external CSS without access to the per-render SVG id. Black-on-terracotta is ~2.8:1, too low; black-on-cream is ~10:1. Keeping those three card types light is the right tradeoff. Documented the reason inline so future tinkering doesn't "fix" it by uniformly re-coloring.
