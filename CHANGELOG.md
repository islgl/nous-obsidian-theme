...
### Changed
- **Handwriting mode heading ornament spacing** — the flower marks (❃ ✿ ❁) before H1/H2/H3 in reading view were crashing into Obsidian's fold-indicator chevron when a heading was collapsible. After two passes correcting direction and overshoot, final tuning settles at `margin-right: 0.3em` on H1 ornament, `0.25em` on H2/H3, and `margin-left: 0.05em` on `.heading-collapse-indicator` — effective gap ≈ 0.3em: clearly distinct, still tight.
