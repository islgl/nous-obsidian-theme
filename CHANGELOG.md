...
### Fixed
- **Mermaid dark-mode palette, take 2** — the light-mode slice rotation (terracotta / tan / deep brown / etc.) bled to mud when dropped onto the `#141413` dark body; the mid-tone browns lost all contrast. Introduced a dedicated dark-mode rotation — coral, golden tan, taupe, apricot, antique gold, warm khaki, burnt sienna — applied via `.theme-dark .mermaid { --nous-slice-N: ...; }`. Same palette now drives pie slices and legend chips in dark. Dark-mode sequence-note surface lifts from `--nous-ivory` (#1c1c1a, too close to body) to `--nous-warm-sand` (#30302e, reads as a card). Gantt task bars bump to 0.55 fill-opacity in dark so terracotta reads as warmth, not as dim salmon.
